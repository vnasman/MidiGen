...
// Each carries its own tip — shown when the preset is picked in the dropdown,
// so the tip box always describes what's actually selected.
const MACHINE_OPTIONS = [
  { value: 'machineBossa',    label: 'Bossa Nova', tip: 'Bossa Nova — rim clave + soft kick, THE organ preset. 110–130 BPM.' },
  { value: 'machineCha',      label: 'Cha-Cha',    tip: 'Cha-Cha — cowbell pulse, guiro, and the namesake double conga on the 4-and. 100–125 BPM.' },
  { value: 'machineRumba',    label: 'Rumba',      tip: 'Rumba — 3-2 son clave, sparse and elegant. 95–115 BPM.' },
  { value: 'machineSamba',    label: 'Samba',      tip: 'Samba — 3-3-2 surdo kick, agogô and shaker 16ths. 95–110 BPM.' },
  { value: 'machineMambo',    label: 'Mambo',      tip: 'Mambo — the cáscara bell drives everything. 90–110 BPM.' },
  { value: 'machineBeguine',  label: 'Beguine',    tip: 'Beguine — dusty ballroom sway, triangle and all. 70–95 BPM.' },
  { value: 'machineSwing',    label: 'Swing',      tip: 'Swing — ride pattern + feathered kick. Set Swing to Triplet for the skip. 90–160 BPM.' },
  { value: 'machineMarch',    label: 'March',      tip: 'March — kick on the beat, snare answers. The oompah button. 100–125 BPM.' },
  { value: 'machineSlowRock', label: 'Slow Rock',  tip: 'Slow Rock — the 6/8-ballad button. Swing on Triplet gives the full 12/8 sway. 60–80 BPM.' },
  { value: 'machineRock',     label: "Rock '78",   tip: "Rock '78 — stiff organ rock, loops forever, no fills. 100–140 BPM." },
  { value: 'machineDisco',    label: "Disco '78",  tip: "Disco '78 — four-on-the-floor with open-hat offbeats, organ style. 110–125 BPM." },
  { value: 'machinePop',      label: "Pop '78",    tip: "Pop '78 — the CR-78 classic Blondie and Phil Collins built on. 100–130 BPM." },
];
...
const RECIPES = {
  ...
  // Note: the 'machine' (Rhythm box) role is resolved directly in applyCombo —
  // its preset dropdown replaces the genre axis, so it has no recipe row here.
};
...
function bindChips() {
  ...
  if (machineSel) {
    ...
    machineSel.addEventListener('change', e => {
      STATE.drumStyle = e.target.value;
      // Keep the tip in sync with the actual selection — not the genre default.
      const opt = MACHINE_OPTIONS.find(o => o.value === e.target.value);
      const tipEl = document.getElementById('preset-tip');
      if (opt && tipEl) tipEl.textContent = opt.tip;
      regenerate();
    });
  }
}
...
// The Rhythm box role is special: its preset dropdown IS the feel selector, so
// the genre axis is bypassed entirely (the Feel row is hidden meanwhile).
function applyCombo(roleId, genreId, { audition = false } = {}) {
  if (roleId === 'machine') {
    STATE.role = 'machine';
    STATE.engine = 'drums';
    if (!MACHINE_STYLE_KEYS.has(STATE.drumStyle)) STATE.drumStyle = 'machineBossa';
    for (const def of SLIDER_DEFS) STATE.sliders[def.id] = ROLE_BASE.machine[def.id];

    document.querySelectorAll('#role-chips .chip').forEach(b =>
      b.classList.toggle('active', b.dataset.role === 'machine'));
    const opt = MACHINE_OPTIONS.find(o => o.value === STATE.drumStyle);
    const tipEl = document.getElementById('preset-tip');
    if (tipEl) tipEl.textContent = opt?.tip ?? '';
...
function comboLabel() {
  ...
  if (STATE.role === 'machine') {
    const preset = MACHINE_OPTIONS.find(o => o.value === STATE.drumStyle)?.label ?? '';
    return `${r} · ${preset}`;
  }
...
function refreshPanelVisibility() {
  ...
  // The Rhythm box replaces the genre axis with its preset dropdown — showing
  // both would be two controls steering the same thing.
  const feelRow = document.getElementById('feel-row');
  if (feelRow) feelRow.style.display = STATE.role === 'machine' ? 'none' : '';
...
function randomize() {
  ...
  if (role === 'machine') {
    // The rhythm box ignores genre — randomize the preset instead.
    STATE.drumStyle = MACHINE_OPTIONS[Math.floor(Math.random() * MACHINE_OPTIONS.length)].value;
  }
...
function ensureDrumKit() {
  ...
  snare.chain(new Tone.Filter(1800, 'bandpass', -12), new Tone.Volume(-4), masterBus);

  const clap = new Tone.NoiseSynth({
    noise: { type: 'pink' },
    envelope: { attack: 0.003, decay: 0.22, sustain: 0 },
  });
  clap.chain(new Tone.Filter(1200, 'bandpass'), new Tone.Volume(-6), masterBus);

  const hat = new Tone.NoiseSynth({
    noise: { type: 'white' },
    envelope: { attack: 0.001, decay: 0.045, sustain: 0 },
  });
  hat.chain(new Tone.Filter(9000, 'highpass'), new Tone.Volume(-12), masterBus);

  const ohat = new Tone.NoiseSynth({
    noise: { type: 'white' },
    envelope: { attack: 0.001, decay: 0.28, sustain: 0 },
  });
  ohat.chain(new Tone.Filter(8000, 'highpass'), new Tone.Volume(-14), masterBus);

  const metal = new Tone.MetalSynth({
    envelope: { attack: 0.001, decay: 0.6, release: 0.1 },
    harmonicity: 5.1, modulationIndex: 24, resonance: 4000, octaves: 1.2,
  });
  metal.chain(new Tone.Volume(-16), masterBus);

  const perc = new Tone.MembraneSynth({
    pitchDecay: 0.02, octaves: 2,
    envelope: { attack: 0.001, decay: 0.12, sustain: 0, release: 0.05 },
  });
  perc.chain(new Tone.Volume(-6), masterBus);
...
// Shared master bus: gain headroom into a limiter so chord stabs (many
// simultaneous voices) never clip or grind. No distortion anywhere — saw
// through a lowpass stays plenty electronic, but clean.
let masterBus = null;
...
async function ensureSynths() {
  ...
  if (!synthMain) {
    ...
    synthMain.chain(new Tone.Filter(1900, 'lowpass', -24), masterBus);
  }
  if (!synthHarm) {
    // Triangle for harmony voices — square + saw together buzzes; triangle
    // sits cleanly underneath the main line.
